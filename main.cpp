...
int main()
{
    ...
    std::cout << "Score: " << score << " / " << maxScore << "test1" << std::endl;

    score += testSimpleBestFit(); // 3
    std::cout << "Score: " << score << " / " << maxScore << "test2" << std::endl;

    score += testComplexBestFit(); // 13
    std::cout << "Score: " << score << " / " << maxScore << "test3" << std::endl;

    score += testNewAllocator(); // 7
    std::cout << "Score: " << score << " / " << maxScore << "test4" << std::endl;

    score += testInvalidAllocate(); // 1
    std::cout << "Score: " << score << " / " << maxScore << "test5" << std::endl;

    score += testRepeatedShutdown(); // 3
    std::cout << "Score: " << score << " / " << maxScore << "test6" << std::endl;

    score += testMaxInitialization(); // 1
    std::cout << "Score: " << score << " / " << maxScore << "test7" << std::endl;

    score += testGetters(); // 2
    std::cout << "Score: " << score << " / " << maxScore << "test8" << std::endl;
...
std::string vectorToString(const std::vector<uint16_t>& vector)
{
    ...
    {
        // Initial staatement --> May overflow before being assigned to wider type
        //vectorString += "] - [" + std::to_string(vector[i]) + ", " + std::to_string(vector[i + 1]);

        // Fix        
        vectorString += "] - [" + std::to_string(vector[i]) + ", " + std::to_string(vector[static_cast<std::vector<uint16_t, std::allocator<uint16_t>>::size_type>(i) + 1]);
    }
    vectorString += "]";
